...
// An hour with this many steps or fewer is counted as sleep/rest.
const int SLEEP_STEP_THRESHOLD = 5;

// Activity intensity is based on average steps per active hour.
...
int getHoursToCheckForDate(int year, int month, int day) {
  ...
  // Future dates should not count as sleep just because they have 0 steps.
  ...
  // For past days, check all 24 hours.
  return 24;
}
...
String getActivityIntensityForDate(int year, int month, int day) {
  ...
  if (totalSteps == 0 && sleepHours == hoursToCheck) {
    return "Sleeping";
  }
...
void handleRoot() {
  ...
  String activityTodayDisplay = getActivityIntensityForDate(todayYear, todayMonth, todayDay);
